...
- 三方库目录结构
  ```
  tpc_c_cplusplus/thirdparty/RHVoice                      # 三方库的目录结构如下
  ├── docs                                                # 三方库相关文档的文件夹
  ├── HPKBUILD                                            # 构建脚本
  ├── HPKCHECK                                            # 测试脚本
  ├── SHA512SUM                                           # 三方库校验文件
  ...
  ├── README_zh.md                                        # 三方库简介
  └── rhvoice_1.8.0_oh_pkg.patch                          # 三方库适配OpenHarmony的patch文件
  ```
...
- 三方库头文件及生成的库，在lycium目录下会生成usr目录，该目录下存在已编译完成的32位和64位三方库
  ...
  ```
  RHVoice/arm64-v8a    RHVoice/armeabi-v7a
  ```
  
- [测试三方库](#测试三方库)

## 应用中使用三方库
- 在IDE的cpp目录下新增thirdparty目录，将RHVoice及依赖库portaudio生成的头文件和库拷贝到该目录下，如下图所示
  
  ![install.dir](./pic/rhvoice_install_dir.png)
...
进入到构建目录执行./src/test/RHVoice-test 运行测试用例（arm64-v8a-build为构建64位的目录，armeabi-v7a-build为构建32位的目录）

```shell
echo "test12345678" > input.txt
./src/test/RHVoice-test -i input.txt -o output.wav
```
