...
- 将libuuid拷贝至tools/main目录下
  ```
  cd tpc_c_cplusplus
  cp thirdparty/libuuid tools/main -rf
  ```
- 在tools目录下编译三方库
...
- 在最外层（cpp目录下）CMakeLists.txt中添加如下语句
  ...
  #将三方库加入工程中
  target_link_libraries(entry PRIVATE ${CMAKE_CURRENT_SOURCE_DIR}/../../../libs/${OHOS_ARCH}/libuuid.so.1)
  #将三方库的头文件加入工程中
  target_include_directories(entry PRIVATE ${CMAKE_CURRENT_SOURCE_DIR}/thirdparty/include/libuuid/${OHOS_ARCH})

  ```
  ![libuuid_usage](pic/libuuid_usage.png)
## 测试三方库
