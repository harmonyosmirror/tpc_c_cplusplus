...
- 三方库目录结构
  ```
  tpc_c_cplusplus/thirdparty/libtiff  #三方库xerces-c的目录结构如下
...
- 将三方库拷贝到OpenHarmony源码的third_party目录下
  ```
  cp ~/tpc_c_cplusplus/thirdparty/libtiff ~/openharmony/third_party -rf
  ```
### 准备三方库源码
```
cd ~/openharmony/third_party/libtiff    #进入三方库目录
git clone https://gitlab.com/libtiff/libtiff.git -b v4.3.0 --depth=1    #下载三方库源码
```
## 系统Rom中引入三方库
...
新增需要编译的组件,在OpenHarmony源码的vendor/hihope/rk3568/config.json文件中，新增需要编译的组件，如下代码段所示，在thirdparty子系统下面新增libtiff组件
...
```
    {
      ...
      "components": [
        ...
        {
          "component": "libtiff",
          "features": ["enable_libtiff_test=true"]
        }
      ]
    }
```
...
- 执行编译
  ...
  ```
- 生成文件的路径，可执行文件和库文件都在out/rk3568/thirdparty/libtiff目录下，同时也打包到了镜像中
## 运行效果
...
- 将原生库测试需要的所有文件打包成libtiff.tar,并拷贝到windows下
- 将文件推送到开发板，在windows命令行进行如下操作
  ```
  hdc_std shell mount -oremount,rw /     #修改系统权限为可读写
  hdc_std file send libtiff.tar /         #将文件包推入开发板
  hdc_std shell                          #进入开发板
  tar xvf libtiff.tar                     #解压
