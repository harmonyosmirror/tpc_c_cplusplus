...
- 三方库目录结构
  ```
  tpc_c_cplusplus/thirdparty/sqlite  #三方库sqlite的目录结构如下
  ├── docs                              #三方库相关文档的文件夹
  ├── HPKBUILD                          #构建脚本
  ├── SHA512SUM                         #三方库校验文件
...
- 将sqlite拷贝至tools/main目录下
  ```
  cd tpc_c_cplusplus
  cp thirdparty/sqlite tools/main -rf
  ```
- 在tools目录下编译三方库
...
- 在最外层（cpp目录下）CMakeLists.txt中添加如下语句
  ```
  #将三方库加入工程中
  target_link_libraries(entry PRIVATE ${CMAKE_SOURCE_DIR}/../../../libs/${OHOS_ARCH}/libsqlite.so.0)
  #将三方库的头文件加入工程中
  target_include_directories(entry PRIVATE ${CMAKE_CURRENT_SOURCE_DIR}/thirdparty/sqlite/${OHOS_ARCH}/include)
  ```
  ![sqlite_usage](pic/sqlite_usage.jpg)
## 测试三方库
