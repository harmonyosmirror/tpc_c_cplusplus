...
- 三方库目录结构
  ```
  tpc_c_cplusplus/thirdparty/axtls #三方库的目录结构如下
  ├── docs                              #三方库相关文档的文件夹
  ├── HPKBUILD                          #构建脚本
  ├── HPKCHECK                          
  ├── SHA512SUM                         #三方库校验文件
...
```
#将三方库加入工程中
target_link_libraries(entry PRIVATE ${CMAKE_CURRENT_SOURCE_DIR}/../../../libs/${OHOS_ARCH}/libaxtls.a)
#将三方库的头文件加入工程中
target_include_directories(entry PRIVATE ${CMAKE_CURRENT_SOURCE_DIR}/thirdparty/axtls/${OHOS_ARCH}/include)
```
  ![thirdparty_usage](pic/usage.png)
## 测试三方库
三方库的测试使用原库自带的测试用例来做测试

进入到构建目录获取,执行ctest，运行测试用例（axtls-arm64-v8a-build为构建64位的目录，axtls-armeabi-v7a-build为构建32位的目录）
