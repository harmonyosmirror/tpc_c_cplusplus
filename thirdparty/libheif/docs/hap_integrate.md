...
- 三方库目录结构
  ...
  ```shell
  tpc_c_cplusplus/thirdparty/libheif  #三方库libheif的目录结构如下
  ├── docs                              #三方库相关文档的文件夹
  ├── HPKBUILD                          #构建脚本
  ├── SHA512SUM                         #三方库校验文件
...
- 将libheif拷贝至tools/main目录下

  ```shell
  cd tpc_c_cplusplus
  cp thirdparty/libheif tools/main -rf
  ```
...
- 三方库头文件及生成的库
  ...
  ```shell
  libheif/arm64-v8a   libheif/armeabi-v7a
  ```
...
- 在IDE的cpp目录下新增thirdparty目录，将编译生成的库拷贝到该目录下，如下图所示
  ...
  ![thirdparty_install_dir](pic/libheif_install_dir.png)
...
- 在最外层（cpp目录下）CMakeLists.txt中添加如下语句
  ...
  ![libheif_usage](pic/libheif_usage.png)
...
&nbsp;![libheif_test](pic/libheif_test.png)
