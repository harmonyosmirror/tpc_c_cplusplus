...
- 三方库目录结构
  ```
  tpc_c_cplusplus/thirdparty/udt        #三方库udt的目录结构如下
  ├── docs                              #三方库相关文档的文件夹
  ├── HPKBUILD                          #构建脚本
  ├── HPKCHECK                          #测试用例执行脚本
  ├── SHA512SUM                         #三方库校验文件
...
- 在IDE的cpp目录下新增thirdparty目录，将编译生成的库拷贝到该目录下，如下图所示
    ...
    ![输入图片说明](pic/udt_install_dir.PNG)
- 在最外层（cpp目录下）CMakeLists.txt中添加如下语句
  ```
  
  #将三方库加入工程中
  target_link_libraries(entry PUBLIC ${CMAKE_CURRENT_SOURCE_DIR}/thirdparty/udt/${OHOS_ARCH}/lib/libudt.a)
  #将三方库的头文件加入工程中
  target_include_directories(${CMAKE_CURRENT_SOURCE_DIR}/thirdparty/udt/${OHOS_ARCH}/include)
