...
- 三方库目录结构
  ...
  ```shell
  tpc_c_cplusplus/thirdparty/stb      #三方库stb的目录结构如下
  ├── docs                              #三方库相关文档的文件夹
...
- 将stb拷贝至tools/main目录下

  ```shell
  cd tpc_c_cplusplus
  cp thirdparty/stb tools/main -rf
  ```
...
- 在IDE的cpp目录下新增thirdparty目录，将编译生成的库拷贝到该目录下，如下图所示
  ...
  ![thirdparty_install_dir](pic/stb_install_dir.png)

- 在最外层（cpp目录下）CMakeLists.txt中添加如下语句

  ```shell
  #将三方库的头文件加入工程中
  target_include_directories(entry PRIVATE ${CMAKE_CURRENT_SOURCE_DIR}/thirdparty/stb/${OHOS_ARCH}/include)
  ```

  ![stb_usage](pic/stb_usage.png)
