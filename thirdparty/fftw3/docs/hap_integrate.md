...
- 三方库目录结构
  ...
  ```shell
  tpc_c_cplusplus/thirdparty/fftw3      #三方库fftw3的目录结构如下
  ├── docs                              #三方库相关文档的文件夹
  ├── HPKBUILD                          #构建脚本
  ├── SHA512SUM                         #三方库校验文件
...
- 将fftw3拷贝至tools/main目录下

  ```shell
  cd tpc_c_cplusplus
  cp thirdparty/fftw3 tools/main -rf
  ```
...
- 三方库头文件及生成的库
  ...
  ```shell
  fftw3/arm64-v8a   fftw3/armeabi-v7a
  ```
- [测试三方库](#测试三方库)
...
- 在IDE的cpp目录下新增thirdparty目录，将编译生成的头文件拷贝到该目录下，将编译生成的三方库全部（动态库名字带版本号和不带版本号的都需要）拷贝到工程的libs目录下，如下图所示
  ...
  ![thirdparty_install_dir](pic/fftw3_install_dir.png)
...
- 在最外层（cpp目录下）CMakeLists.txt中添加如下语句
  ...
  target_link_libraries(entry PRIVATE
	${CMAKE_CURRENT_SOURCE_DIR}/../../../libs/${OHOS_ARCH}/libfftw3f.so
	${CMAKE_CURRENT_SOURCE_DIR}/../../../libs/${OHOS_ARCH}/libfftw3f_threads.so
	)
  #将三方库的头文件加入工程中
  target_include_directories(entry PRIVATE ${CMAKE_CURRENT_SOURCE_DIR}/thirdparty/fftw3/${OHOS_ARCH}/include)
  ```

  ![fftw3_usage](pic/fftw3_usage.png)

## 测试三方库

三方库的测试使用原库自带的测试用例来做测试，[准备三方库测试环境](../../../tools/README.md#ci环境准备)

进入到构建目录,执行如下命令: make check-local -C tests（arm64-v8a-build为构建64位的目录，armeabi-v7a-build为构建32位的目录）

&nbsp;![fftw3_test](pic/fftw3_test.png)
